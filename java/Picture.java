...
public final class Picture implements ActionListener {
   ...
   /**
    ...
    public void show() {
        ...
        if (frame == null) {
            ...
            JMenu menu = new JMenu(this.filename);
            menuBar.add(menu);
   ...
   /** 
    ...
    public Picture scale(int factor) {
        ...
        Picture bigger = new Picture(this.width * factor, this.height * factor);
        bigger.filename = "x" + factor + this.filename;
        bigger.isOriginUpperLeft = this.isOriginUpperLeft;
        for (int col = 0; col < bigger.width(); col++)
            for (int row = 0; row < bigger.height(); row++)
                bigger.image.setRGB(col, row, this.image.getRGB(col/factor, row/factor));
        return bigger;
    }
